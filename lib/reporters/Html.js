define([
	...
	function HtmlReporter(config) {
		...
		function getIndentLevel(node) {
			var titleNode = node.childNodes[1];
			var classes = titleNode.className.split(' ');
			var indentLevel = 0;
			classes.forEach(function (className) {
				var index = className.indexOf('indent');
				if (index !== -1) {
					indentLevel = parseInt(className.slice(index + 'indent'.length), 10);
				}
			});
		...
		this.suiteStart = function (suite) {
			...
			rowNode.addEventListener('click', this._closeSuiteHandler);
			rowNode.appendChild(cellNode);
		...
		this._closeSuiteHandler = function () {
			var open = this.getAttribute('open') !== 'true';
			this.setAttribute('open', open);

			var indentLevel = getIndentLevel(this);
			var node = this.nextSibling;
			while (node) {
				if (node.className.indexOf('suite') !== -1 && getIndentLevel(node) <= indentLevel) {
					return;
				}

				if (open) {
					addClass(node, 'display-none');
				}
				else {
					removeClass(node, 'display-none');
				}
